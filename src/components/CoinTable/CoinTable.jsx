...
function CoinTable() {
  ...
  const filteredList = coinData.filter(item => {
    return filterCoin.toLowerCase() === "" ?
      item :
      item.name.toLowerCase().includes(filterCoin.toLowerCase())
  });

  //calculate indexes for pagination
  let indexOfLastPost = currentPage * coinsPerPage;
  if (indexOfLastPost > filteredList.length) {
    indexOfLastPost = filteredList.length
  }
              ...
              (<>
                {
                  filteredList.length > 0 ?
                    filteredList.slice(indexOfFirstPost, indexOfLastPost).map(items => {
                      return (
                        <>
                          <tbody>
                            <tr key={items.id}>
